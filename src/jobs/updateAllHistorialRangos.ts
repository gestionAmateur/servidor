...
export const updateAllHistorialRangos = () => {
  cron.schedule('0 3 * * */1', async () => {
    ...
    try {
      const cuentas = await cuentaInvocadorService.getAllCuentaInvocadores();
      for (const cuenta of cuentas) {
        console.log(cuenta)
        await historialRangosService.createOrUpdateHistorialRangos(cuenta);
      }
      console.log('Historial de rangos actualizado para todas las cuentas de invocador.');
    } catch (error) {
